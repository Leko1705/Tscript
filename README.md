...
- <b>Minor version:</b> the minor version this bytecode supports
- <b>Major version:</b> the major version this bytecode supports
- <b>module name:</b> the name of this module
- <b>Entry point: </b> a reference to a constant pool entry that contains
the reference to the first function to execute (main function)
- <b>Global register amount:</b> the amount of global registers that are 
required for dealing with global variables.
...
The constant pool begins with a 2 byte number encoding the amount of
entries in the pool.
...
- ARRAY (n * 2 bytes referencing to any entry in the constant pool)
- DICTIONARY (n * 2 * 2 bytes for key and value, each referencing any entry 
in the constant pool)
- VFUNCTION (name + ID referring to a function in the bytecode)
- NFUNCTION (name referencing code that is not contained in the bytecode)
- TYPE (2 bytes referencing the type-ID)
- UTF8 (n bytes + null-terminator)
- MODULE (n bytes + null-terminator)

#### Functions
The functions section begins with a 2 byte number encoding the amount
of functions.<br>
Each functions head is encoded as follows:<br>
<b>NAME | PARAM_LIST | STACK_SIZE | LOCAL_REGISTER_AMOUNT | INSTRUCTION_AMOUNT</b>
<br>
where:<br>
- <b>NAME:</b> is the name of the function
- <b>PARAM_LIST:</b> a list of parameters. Each parameter is encoded by starting with its name,
the end of the name can be determined with a null-terminator, and its 2 byte default-value-address,
referencing to a constant in the constant pool.
...
- <b>INSTRUCTION_AMOUNT:</b> the amount of instructions in this function
...
#### Instruction Set
<b>Note:</b> `op-stack` here stands for `operand-stack` (see more in the Virtual machine specifications)
...
| 3      |     LOAD_CONST     |               2 |                                                                                                                                                                                                                                                                                              Loads the constant from the constant pool indexed by the 2 byte arguments. |                                                                                 `[] -> [c]` |
| 4      |     PUSH_THIS      |               0 |                                                                                                                                                                                                                                                                                                                                        Pushes `this` onto the op-stack. |                                                                              `[] -> [this]` |
| 5      |        POP         |               0 |                                                                                                                                                                                                                                                                                                                              Removes the top element from the op-stack. |                                                                               `[top] -> []` |
| 6      |      NEW_LINE      |               4 |                                                                                                                                                                                                                                                                             Marks that a new line is reached. The line is encoded as a 4 byte integer by the arguments. |                                                                                  `[] -> []` |
| 7      |    LOAD_GLOBAL     |               1 |                                                                                                                                                                                                                                                                                          Loads from the global register of the current module, indexed by the argument. |                                                                       `[] -> [some_global]` |
| 8      |    STORE_GLOBAL    |               1 |                                                                                                                                                                                                                                        Removes the top element from the op-stack and stores it into the global register of the current module, indexed by the argument. |                                                                       `[some_global] -> []` |
| 9      |     LOAD_LOCAL     |               1 |                                                                                                                                                                                                                                                                Removes the top element from the op-stack and stores it into the local register indexed by the argument. |                                                                        `[] -> [some_local]` |      
| 10     |    STORE_LOCAL     |               1 |                                                                                                                                                                                                                                                                Removes the top element from the op-stack and stores it into the local register indexed by the argument. |                                                                        `[some_local] -> []` |
| 11     |    LOAD_MEMBER     |               2 |            Removes the top element from the op-stack and accesses its member my name. The name is accessed from the constant pool by the argument. The accessed member is then pushed onto the op-stack. An error is thrown if the given member does not exist in this object or if it is not accessible from this scope. An error is thrown if the field is immutable. |                                                                      `[obj] -> [obj.field]` |
| 12     |    STORE_MEMBER    |               2 |                Removes the top two elements from the op-stack and stores the second removed element as a member into the first one by name. The name is accessed from the constant pool by the argument. An error is thrown if the given member does not exist in this object or if it is not accessible from this scope. An error is thrown if the field is immutable. |                                                                     `[obj, to_store] -> []` |
| 13     |  LOAD_MEMBER_FAST  |               1 |                                                                                                                                                                                                                                                         Accesses a field of `this` by index. The field is then pushed onto the stack. The index is the 1 byte argument. |                                                                        `[] -> [this.field]` |
| 14     | STORE_MEMBER_FAST  |               1 |                                                                                                                                                                                                                                                Removes the top element from the op-stack and stores it in a field of `this` by index. The index is the 1 byte argument. |                                                                          `[to_store] -> []` |
| 15     |    LOAD_STATIC     |               2 |                                                  Loads a static field by name and pushes it onto the op-stack. The accessed object is `this`. The name is accessed from the constant pool by the argument. An error is thrown if the given member does not exist in this type or if it is not accessible from this scope. An error is thrown if the field is immutable. |                                                               `[] -> [static_field_of_obj]` |
| 16     |    STORE_STATIC    |               2 |                                              Removes the top element from the op-stack and stores it into the static field of `this` by name. The name is accessed from the constant pool by the argument. An error is thrown if the given member does not exist in this type or if it is not accessible from this scope. An error is thrown if the field is immutable. |                                                               `[static_field_of_obj] -> []` |
| 17     |   CONTAINER_READ   |               0 |                                                                                                    Removes the top two elements from the op-stack and accesses the first removes as a container with the second one as the key. The accessed value is then pushed onto the op-stack. An error is thrown if the accessed object is not accessible via the `[]` operator. |                                                               `[container, key] -> [value]` |
| 18     |  CONTAINER_WRITE   |               0 |                                                                                                                              Removes the top three elements from the op-stack and stores the third one (the value) into the first one (the container) with the second one as the key. An error is thrown if the accessed object is not writeable via the `[]` operator. |                                                             `[container, key, value] -> []` |
| 19     | LOAD_ABSTRACT_IMPL |               2 |                                                                                                                                                                                   Loads the implementation of an as abstract defined function by name. The name is accessed from the constant pool by the argument. Throws an error if the implementation is not found. |                                                            `[] -> [abstract_function_impl]` |
| 20     |        ADD         |               0 |                                                                                                                                                               Removes the top two elements from the op-stack and performs an addition on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 21     |        SUB         |               0 |                                                                                                                                                             Removes the top two elements from the op-stack and performs a subtraction on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 22     |        MUL         |               0 |                                                                                                                                                          Removes the top two elements from the op-stack and performs a multiplication on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 23     |        DIV         |               0 |                                                                                                                                                                Removes the top two elements from the op-stack and performs a division on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 24     |        IDIV        |               0 |                                                                                                                                                       Removes the top two elements from the op-stack and performs an integer division on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 25     |        MOD         |               0 |                                                                                                                                                        Removes the top two elements from the op-stack and performs a modulo operation on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 26     |        POW         |               0 |                                                                                                                                                                                Removes the top two elements from the op-stack and powers them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                              `[exponent, base] -> [result]` |
| 27     |        AND         |               0 |                                                                                                                                                          Removes the top two elements from the op-stack and performs an and operation on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 28     |         OR         |               0 |                                                                                                                                                           Removes the top two elements from the op-stack and performs an or operation on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 29     |        XOR         |               0 |                                                                                                                                                           Removes the top two elements from the op-stack and performs a xor operation on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 30     |        NOT         |               0 |                                                                                                                                                               Removes the top element from the op-stack and performs a logical negation on it. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                      `[bool] -> [not bool]` |
| 31     |         LT         |               0 |                                                                                                                               Removes the top two elements from the op-stack and compares them if the left element is less than the right one. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 32     |         GT         |               0 |                                                                                                                            Removes the top two elements from the op-stack and compares them if the left element is greater than the right one. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 33     |        LEQ         |               0 |                                                                                                                      Removes the top two elements from the op-stack and compares them if the left element is less or equal than the right one. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 34     |        GEQ         |               0 |                                                                                                                   Removes the top two elements from the op-stack and compares them if the left element is greater or equal than the right one. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 35     |        SLA         |               0 |                                                                                                                                       Removes the top two elements from the op-stack and performs a left arithmetical shift operation on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 36     |        SRA         |               0 |                                                                                                                                      Removes the top two elements from the op-stack and performs a right arithmetical shift operation on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 37     |        SRL         |               0 |                                                                                                                                           Removes the top two elements from the op-stack and performs a right logical shift operation on them. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                 `[right, left] -> [result]` |
| 38     |       EQUALS       |               0 |                                                                                                                                                                                                                                       Removes the top two elements from the op-stack and compares them for their equality. The result is then pushed onto the op-stack. |                                                                 `[right, left] -> [result]` |
| 39     |     NOT_EQUALS     |               0 |                                                                                                                                                                                                                                     Removes the top two elements from the op-stack and compares them for their inequality. The result is then pushed onto the op-stack. |                                                                 `[right, left] -> [result]` |
| 40     |        NEG         |               0 |                                                                                                                                                                       Removes the top element from the op-stack and performs a negation on it. The result is then pushed onto the op-stack. An error is thrown if the operation is not applicable on the two arguments. |                                                                     `[number] -> [+number]` |
| 41     |        POS         |               0 |                                                                                                                                     Removes the top element from the op-stack and signs it with `+`. The result is then pushed onto the op-stack. Effectively this operation has no effect. An error is thrown if the operation is not applicable on the two arguments. |                                                                     `[number] -> [-number]` |
| 42     |      GET_TYPE      |               0 |                                                                                                                                                                                                                                                                                        Removes the top element from the op stack and pushes its type onto the op stack. |                                                                    `[obj] -> [type_of_obj]` |
| 43     |     MAKE_ARRAY     |               1 |                                                                                                                                                                                                                                           Removes n top elements of the op-stack and stores them into a new array. The first element is the first element in the array. |                                                                  `[1, 2, 2] -> [[1, 2, 3]]` |
| 44     |     MAKE_DICT      |               1 |                                                                                                                           Removes the 2 * n elements from the op-stack. for each 'tuple' the first element represents the key and the second one the value. Each pair is then stored into a new dictionary. The first removed tuple is the first one in the dictionary. |                                              `["hi", 1, "bye", 2] -> [{"hi": 1, "bye", 2}]` |
| 45     |     MAKE_RANGE     |               0 |                                                                                                                                     Removes the top two elements from the op-stack and creates a new range from them. The first removed element is the 'from' value and the second value the 'to' value. Throws an error if one of the two arguments is not an integer. |                                                                   `[from, to] -> [from:to]` |
| 46     |        GOTO        |               2 |                                                                                                                                                                                                                                                                                                     jumps to the absolute address given by the 2 byte integer argument. |                                                                                  `[] -> []` |
| 47     |   BRANCH_IF_TRUE   |               2 |                                                                                                                                                                                                              Removes the top element from the op-stack and checks for its truth value. If it is true an absolute jump to the given 2 byte integer address is performed. |                                                                                  `[] -> []` |
| 48     |  BRANCH_IF_FALSE   |               2 |                                                                                                                                                                                                             Removes the top element from the op-stack and checks for its truth value. If it is false an absolute jump to the given 2 byte integer address is performed. |                                                                                  `[] -> []` |
| 49     |      GET_ITR       |               0 |                                                                                                                                                                                                                Removes the top element from the op-stack and pushes it's iterator object onto the stack. Throws an error if the given object is not an iterable object. |                                                                     `[obj] -> [itr_of_obj]` |
| 50     |     BRANCH_ITR     |               2 |                                                                                                                                                      Peeks the top element of the op-stack as an iterator object and checks if it has a next element. If not the iterator is removes from the op-stack and an absolute jump to the 2 byte integer address is performed. |                                        if true: `[itr] -> [itr]`<br>if false: `[itr] -> []` |
| 51     |      ITR_NEXT      |               0 |                                                                                                                                                                                                                                                              Peeks the top element from the op-stack as an iterator object and pushes its next value onto the op-stack. |                                                          `[itr] -> [next_itr_element, itr]` |
| 52     |     ENTER_TRY      |               1 |                                                                                                                                                                                                                                                                       Maks that an try block has been entered by storing the escape address in case of an caught error. |                                                                                  `[] -> []` |
| 53     |     LEAVE_TRY      |               0 |                                                                                                                                                                                                                                                                                                                                          Unmarks the entered try block. |                                                                                  `[] -> []` |
| 54     |       THROW        |               0 |                                                                                                                                                                                     Removes the top element from the op-stack and throws this element as an exception. If the backtracking ends in a try block the thrown value is pushed back onto the (new) op-stack. |                                                                           `[throwed] -> []` |
| 55     |        CALL        |               1 | Removes the top element from the op-stack as the function and n further elements as the parameters The amount of parameters is specified by the 1 byte argument. Throws an error if the called object is not callable, if the argument amount does not match the parameter count or if a non existent parameter is assigned. See CALL instruction for potential errors. |                                                                        `[p1, p2, p3] -> []` |
| 56     |   WRAP_ARGUMENT    |               2 |                                                                                                                                                                                                                     Removes the top element from the op-stack and pushes it as a tagged element. The tag is loaded from the constant pool by the 2 byte index argument. |                                                                                `[p] -> [p]` |
| 57     |       RETURN       |               0 |                                                                                                                                                                                                             Removes the top element from the op-stack and returns it from the function. The returned element is then pushed onto the op-stack of the previous function. | in from returned function: `[return_value] -> []`<br>in to returned: `[] -> [return_value]` |
| 58     |     CALL_SUPER     |               1 |                                                                                                                                                                                    Removes the top n elements from the op-stack and passes them to the function call of the super constructor of `this`. The amount of parameters is determined by the 1 byte argument. |                                                                        `[p1, p2, p3] -> []` |
| 59     |       IMPORT       |               2 |                                                                                                                                  Loads a new or exising module into the global name registers but also into the module area for later access. The module name is loaded from the constant pool by the given 2 byte index. Throws an error if the module does not exist. |                                                                                  `[] -> []` |
| 60     |    FROM_IMPORT     |               4 |                       Loads a new or exising module into the module area for later access and loads the specified member into the global name registers. The module name is loaded from the constant pool by the given first 2 byte index where the member name is loaded from the second 2 bytes. Throws an error if the module or its accessed member does not exist. |                                                                                  `[] -> []` |
| 61     |        USE         |               2 |                                                                                                                                                                  Removes the top element from the op-stack as a module or namespace and stores all its members into the global name registers. Throws an error if the given object is neither a module nor a namespace. |                                                                                  `[] -> []` |
| 62     |     LOAD_NAME      |               2 |                                                                                                                                                                                          Loads an element from the global name registers. The name is loaded from the constant pool by the 2 byte index argument. Throws an error if the looked up name does not exist. |                                                                                  `[] -> []` |
| 63     |    BREAK_POINT     |               0 |                                                                                                                                                                                                                                                                                                         Pauses the whole Virtual Machine and listens for debug actions. |                                                                                  `[] -> []` |
...
- <b>Global Index Registers</b> representing indexed global variables. The amount of global variables is fix.
Global variables are accessed by index.
- <b>Global Name Registers</b> for holding global variables that were added with the `use` statement. 
