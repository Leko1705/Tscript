...
| 4      |  LOAD_BUILTIN   |               2 |                                                                                                                                                                                                                                                                                 Loads a builtin object of any type onto the op-stack. The object is loaded by the given 2 byte index. |                                                                  `[] -> [virtual_function]` |
...
| 15     |  LOAD_INTERNAL  |               2 |                                                                                                                                                               Performs an internal member read operation of `this` by name and pushes the accessed element onto the stack. The name is loaded by the 2 byte argument from the constant pool. Visibilities are not taken into account. |                                                                        `[] -> [this.field]` |
| 16     | STORE_INTERNAL  |               2 |                                                                                                                                     Performs an internal member write operation by removing the top element from the op-stack and stores it as a member of `this` by name. The name is loaded by the 2 byte argument from the constant pool. Visibilities are not taken into account. |                                                                          `[to_store] -> []` |
